	...
	describe('chai-lodash', function() {
		...
		chai.use(function (chai, utils) {
			...
			chai.Assertion.addMethod('fail', function (message) {
				...
				try {
					...
					this.assert(err instanceof chai.AssertionError, 'expected #{this} to fail, but it threw ' + inspect(err));
					this.assert(err.message === message, 'expected #{this} to fail with ' + inspect(message) + ', but got ' + inspect(err.message));
					return;
				}
		...
		describe('matchers', function() {
			...
			describe('.jsonOf', function() {
				...
				describe('when given two objects that JSON stringify to similiar results', function() {
					...
					describe('when negated', function() {
						...
						it('fails', function() {
							...
							}).should.fail('expected ' + inspect(self.appleJSON) + ' not to be like JSON ' + inspect(self.pearJSON));
							(function(){
								self.pearJSON.should.not.be.jsonOf(self.apple);
							}).should.fail('expected ' + inspect(self.pearJSON) + ' not to be like JSON ' + inspect(self.appleJSON));
						});
				...
				describe('when given two objects that JSON stringify to dissimilar results', function() {
					it('should fail', function() {
						...
						}).should.fail('expected ' + inspect(self.appleJSON) + ' to be like JSON ' + inspect(self.orangeJSON));
						(function(){
							self.orangeJSON.should.be.jsonOf(self.apple);
						}).should.fail('expected ' + inspect(self.orangeJSON) + ' to be like JSON ' + inspect(self.appleJSON));
					});
			...
			describe('.like', function() {
				...
				describe('deep likeness', function () {
				...
				function containmentAssertions(){
					...
					describe('for numbers', function() {
						...
						it('fails when items are not there', function() {
							...
							}).should.fail('expected ' + inspect(subject) + ' to contain one thing like ' + inspect(arrMissing));
						});
						...
						it('negated fails when items are there', function() {
							...
							}).should.fail('expected ' + inspect(subject) + ' not to contain one thing like ' + inspect(numNeedle));
						});
					});
					...
					describe('for strings', function() {
						...
						it('negated fails when items are there', function() {
							...
							}).should.fail('expected ' + inspect(subject) + ' not to contain one thing like ' + inspect(strNeedle));
						});
					});
					...
					describe('for arrays', function() {
						...
						it('negated fails when items are there', function() {
							...
							}).should.fail('expected ' + inspect(subject) + ' not to contain one thing like ' + inspect(arrNeedle));
						});
					});
					...
					describe('for objects', function() {
						...
						it('negated fails when items are there', function() {
							...
							}).should.fail('expected ' + inspect(subject) + ' not to contain one thing like ' + inspect(objNeedle));
						});
					});
				}
			});
