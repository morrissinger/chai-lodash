(function(plugin){
	...
	chai.Assertion.addMethod('like', function(expected){
		...
		this.assert(_.isEqual(obj, expected), 'expected #{this} to be like #{exp}', 'expected #{this} not to be like #{exp}', expected, obj, true);
	});
	...
	chai.Assertion.addMethod('jsonOf', function(expected){
		...
		this.assert(_.isEqual(obj, expectedAsJSON), 'expected #{this} to be like JSON #{exp}', 'expected #{this} not to be like JSON #{exp}', expectedAsJSON, obj, true);
	});
	...
	chai.Assertion.addMethod('containOneLike', function(expected){
		...
		if (!_obj.isObject() && !_obj.isArray()) {
			this.assert(false, 'expected #{this} to be an array, object, or string', 'expected #{this} to be an array, object, or string');
		}
		...
		this.assert(found, 'expected #{this} to contain one thing like #{exp}', 'expected #{this} not to contain one thing like #{exp}', expected);
	});
